...
export function ExportButton({
  ...
  const SaveProject = (e) => {
    ...
    // TODO: separate out compression/serialization so that we don't have to just undo it.
    const data = JSON.parse(LZString.decompress(AppState.serialize({
  ...
  const LoadProject = (e) => {
    ...
    reader.onload = async (e) => {
      ...
      console.log(data);
      dispatch(
        AppState.actions.loadState({
          dataTable: DataTable.fromObject(data.dataTable),
          userDefinedStates: data.userDefinedStates.map((o) =>
            objectToState(o)
          ),
          summaryTables: data.summaryTables.map((s) => ({
            state: objectToState(s),
          })),
  ...
  return (
    <NavDropdown title="Export Data">
      <NavDropdown.Item onClick={ExportCSV}>
        CSV (Current Table)
      </NavDropdown.Item>
      <NavDropdown.Divider></NavDropdown.Divider>
      <NavDropdown.Item onClick={SaveProject}>Save Project</NavDropdown.Item>
      <NavDropdown.Item onClick={() => lp.current.click()}>
        Load Project
      </NavDropdown.Item>
      <input
        type="file"
