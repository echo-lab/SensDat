...
import "../../styles/spatial-temporal/condition-state-pane.css";
...
export function ConditionStatePane({ dataTable, dispatch }) {
  ...
  const onCreate = () => {
    dispatch(
      actions.CreateConditionState(
        new ConditionState(expressionValue, filteredData.rows, dataTable.rows)
      )
    );
  };
...
function AutoCompleteBox({
  ...
  const handleChange = (event) => {
    ...
    try {
      ...
      setFilteredData(
        DataTable.fromRowsCols(
          filterList(dataTable.rows, input, allowedTokens),
          dataTable.cols
        )
      );
      setResponse("Correct");
  ...
  const handleSelectSuggestion = () => {
    ...
    const newValue = input.substring(0, lastWordStartIndex) + suggestion + " ";
...
export const filterList = (list, expression, allowedIdentifiers) => {
  ...
  allowedIdentifiers = allowedIdentifiers.filter(
    ...
    switch (node.type) {
      ...
      case "Identifier":
        ...
        if (!allowedIdentifiers.includes(identifier)) {
          const match = allowedIdentifiers.find(
            (i) => i.toLowerCase() === identifier.toLowerCase()
          );
          if (match) {
            identifier = match;
          } else {
  ...
  const filteredList = list.filter((obj) =>
    evaluateNode(ast.body[0].expression, obj)
  );
  return filteredList;
};
...
function CFVirtualizedTable({ dataTable, highlightRows, applyHighlight }) {
  ...
  const RenderRow = useCallback(
    ({ index, style }) => {
      ...
      if (applyHighlight) {
        // Only check for highlighting if applyHighlight is true
        for (let range of highlightRows) {
          if (index + 1 >= range[0] && index + 1 <= range[1]) {
            // Index + 1 here is to fit Order number with index number
            isHighlighted = true;
            break;
          }
        }
