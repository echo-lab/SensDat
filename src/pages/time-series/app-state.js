...
export function serialize({
  ...
  let res = {
    ...
    //timeSeriesGraphs: timeSeriesGraphs.map((graph) => graph.asObject()),
    defaultDataTransform:
...
export async function deserialize(serializedState) {
  ...
  return {
    ...
    timeSeriesGraphs: data.timeSeriesGraphs.map((graph) =>
      objectToState(graph)
    ),
    defaultDataTransform:
...
actionHandlers["createTimeSeriesGraph"] = (state, activeVizTab) => {
  ...
  return {
    ...
    dataTable: state.dataTable,
    uiState: UIState.Default,
    timeSeriesGraphs: state.timeSeriesGraphs.concat(activeVizTab),
  };
};
